...
router.get('/api/archive-posts', async (req, res) => { 
    try {
        ...
        let queryParams = [];
        let whereClause = 'WHERE p.is_pinned = false'; // Sabitlenmişleri gösterme

        if (category_id) {
            queryParams.push(category_id);
            whereClause += ` AND p.category_id = $${queryParams.length}`;
        }
        ...
        const queryText = `
            ...
            ${whereClause}
            ORDER BY p.created_at DESC
            OFFSET 5; -- Ana sayfadaki 5'i atla
        `;

        const archivedPosts = await pool.query(queryText, queryParams);
